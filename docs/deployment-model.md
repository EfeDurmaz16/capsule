...
The Azure Container Apps adapter maps `job.run` to a manual Container Apps Job create/update followed by the ARM job start operation. It reads execution state through the ARM job execution resource and maps `job.cancel` to the ARM stop execution operation. It does not yet stream Log Analytics output or collect artifacts.
...
The Azure Container Apps adapter maps `service.deploy`, `service.status`, and `service.delete` to ARM Container App create/update, get, and delete. Managed environment, ingress, registry, identity, secrets, revisions, and scaling stay visible in specs and metadata rather than being flattened into a fake generic service model. ARM exposes revision activate/deactivate operations, but Capsule does not expose a generic service rollback operation in this adapter.
