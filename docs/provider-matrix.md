...
| Cloud Run | `@capsule/adapter-cloud-run` | unsupported | native | native | unsupported | unsupported | unsupported | unsupported | Cloud Run Jobs and Services are implemented via Admin API v2; service update uses `services.patch` with `forceNewRevision`, rollback migrates traffic to an explicit or selected previous revision, job/service logs use Cloud Logging `entries:list`; IAM/public access and preview orchestration are not faked. |
| Cloudflare Workers | `@capsule/adapter-cloudflare` | unsupported | unsupported | unsupported | native | unsupported | unsupported | unsupported | Worker module upload, route creation, version upload, and rollback via Workers Deployments are implemented; secrets, logs, provider-specific bindings, and gradual traffic-split release remain explicit future capabilities. |
| Vercel | `@capsule/adapter-vercel` | unsupported | unsupported | unsupported | native | unsupported | unsupported | unsupported | Inline deployment creation, deployment status, alias release, bounded deployment event logs, and project runtime logs are implemented; runtime logs require `projectId`, follow streaming is rejected, and service/preview wrappers, large file upload/SHA flow, env, routes, and rollback remain explicit future capabilities. |
