...
| Cloud Run | `@capsule/adapter-cloud-run` | unsupported | native | native | unsupported | unsupported | unsupported | unsupported | Cloud Run Jobs and Services are implemented via Admin API v2; job/service logs use Cloud Logging `entries:list`; IAM/public access and preview orchestration are not faked. |
