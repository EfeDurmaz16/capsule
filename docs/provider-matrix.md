...
| Kubernetes | `@capsule/adapter-kubernetes` | unsupported | native | native | unsupported | unsupported | unsupported | unsupported | Jobs, Deployments, and Services are implemented through the official Kubernetes client; sandbox/machine wrappers, logs/ingress/rollout are explicit future work. |
