...
| Kubernetes | `@capsule/adapter-kubernetes` | unsupported | native | native | unsupported | unsupported | unsupported | unsupported | Jobs, Deployments, Services, and selector-based pod logs are implemented through the official Kubernetes client; sandbox/machine wrappers, ingress, and rollout remain explicit future work. |
