...
| Vercel | `@capsule/adapter-vercel` | unsupported | unsupported | unsupported | native | unsupported | unsupported | unsupported | Inline deployment creation, deployment status, alias release, deployment event logs, and project runtime logs are implemented; service/preview wrappers, large file upload/SHA flow, env, routes, and rollback remain explicit future capabilities. |
